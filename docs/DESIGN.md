...
The aligner, the ingest, the detectors, the stored baselines, the simulator, the evaluation, the HTTP API, the web app with its source switch, and the `simulate-lot` and `report` commands are built. Everything in this design exists; what is left is deciding when the repository goes public, and the open questions above.
