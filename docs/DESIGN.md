...
# Store one seeded synthetic lot with five known faults, and the clean training lots its baseline learns from. A rerun adds zero rows.
...
| Run page, mark good or bad | `PUT /api/runs/{runId}/label` with `{"label": "BAD"}`, then `GET /api/refreshes/{refreshId}` until it is done |
...
| Lot list, predicted depth | `GET /api/reports/depth-model?set=EIGHTY_NINE_POINT` |
| Wafer page, predicted depth | `GET /api/runs/{runId}/depth?set=EIGHTY_NINE_POINT` |
| Run page, whether relabels are allowed | `GET /api/settings` |
...
{ "baseline": { "id": 1, "goodRuns": 30, "k": 6.0, "n": 5, "runZ": 5.0 },
  "runs": [ { "id": 55, "key": "Day_2024_08_01_Wafer_05", "lotId": 6, "lotNo": 6, "positionInLot": 5,
              ...
              "limitFlags": 25, "deviationFlags": 0, "stuckFlags": 0, "persistentZ": 18.810440063476562,
...
A relabel saves the label and answers 202 at once with a refresh, which the Location header names: `GET /api/refreshes/{refreshId}` reports it `RUNNING`, then `DONE` with what it did to the baseline, or `FAILED` with why. The refreshes run one at a time on a thread of their own, so two fits of a source never race to store the same baseline and a small host never holds two fits in memory; a relabel that lands during a refresh gets its own, queued behind it, which reads the newer label. On the public data, marking a flagged wafer GOOD answered in 99 ms and its refresh was done 12.3 s later, a refit and rescoring all 96 wafers; putting it back to AUTO answered in 15 ms and was done within the first poll, because the old baseline was found by its fingerprint. The run page polls every half second, shows what the refit did, and reloads the run whatever the refit's fate, so it always shows the label that was saved. The last 200 refreshes are remembered in memory until the app restarts; one lost to a restart is redone by the next relabel, ingest or simulate-lot, because a refresh converges from any state. Errors are RFC 9457 problem details: 404 for an unknown run or channel, 400 for a bad parameter or label. A server started with `CHAMBERWATCH_READ_ONLY=true`, as a public demo is, refuses every relabel with a 403 before anything is written, and `GET /api/settings` says so, so the run page shows the label without the buttons. The jar also serves the built web app from its static resources: a link to one of the app's pages, `/runs/...` or `/lots/...`, answers with its `index.html`, and over HTTP the actuator answers `/actuator/health` and nothing else. A synthetic run's detail also carries `injectedFault`, the kind, channel, start, end and size the simulator put in, which is null for public runs and clean synthetic ones. Every assessment and runs-table row counts `stuckFlags` beside the limit and deviation flags, and each channel carries its `holds` with their times and its `longestHold`, so the run page can put a stuck sensor next to the excursions. A run's detail also carries `measuredDepth`: for each measurement set the wafer is in, the 89-point set first, its mean depth, the number of sites, and how much shallower it is than the mean of its lot's first three wafers, counted the way the drift report counts depth loss. The run page shows it as a stat beside the flags: wafer 5 of lot 6, flagged 25 times on the load capacitor, etched 43.69 µm, 0.31 µm shallower than its lot began. A simulated wafer has no measurement, so the list is empty and the stat is left out.

The `Dockerfile` at the repository root builds one image: Node builds the web app, Maven packages the jar with the app in its static resources, and a JRE 21 image runs it with `-XX:MaxRAMPercentage=75`, so a container memory limit bounds the heap. `compose.yaml` starts it next to PostgreSQL 18 with a 64 MB shared cache. With `CHAMBERWATCH_BOOTSTRAP=true` the web server, once up, fills an empty database on a thread of its own: it fetches the five public files the ingest reads from Zenodo, each checked against the committed MD5 list (a file already there with the right checksum is kept, and a download takes its name only once its checksum matches), runs the ingest, then stores the simulated demo lot of the README, seed 7, lot 901. It does only what is missing, judged from the ingest ledger and the demo lot's run keys, and refreshes a source it finds loaded, so a restart with the data in place writes nothing and a start after a crash picks up where the last one stopped. The command-line commands never bootstrap, since they start Spring without a web server. Each attempt runs on the relabel queue's thread, so in the server process one thread at a time writes baselines and assessments, and a relabel during a first-start load waits for it. An attempt that fails, most often Zenodo or the network, is tried again after 1, 5, 15 and 60 minutes, and each file's download is bounded at 5 minutes, headers and body together. `compose.yaml` caps the app at 640 MB and PostgreSQL at 256 MB, what a 1 GB machine can give them: from empty volumes under those caps the first start's load took 68 to 74 s here over two runs (fetch 17 to 18 s, ingest 35 to 37 s, demo lot 15 to 20 s) and peaked at 337 to 352 MiB and 195 to 203 MiB, and a restart with the data in place finished its bootstrap within 150 ms of the server starting, having done nothing.

The lot call returns every channel that has a good-run band on the phase mean. Each comes with the lot's wafers in position order, the fit of the wafers so far at each one, the drift state that fit gives, and the state as of the last wafer. Channels out of the band or projected to leave it come first. On the public data every lot call after the first answers in under 25 ms, and the states as of each lot's last wafer are exactly the table in [DATA.md](DATA.md#flags-and-measured-depth). The report averages each run's drift score by position in lot, next to mean depth and the depth lost since the lot's first 3 wafers, for each measurement set. The depth-model call serves the leave-one-lot-out comparison of [DEPTH.md](DEPTH.md): the error of the telemetry model and of the two simpler guesses, the strongest standardized coefficients, and measured against predicted depth by wafer position. A run's own prediction comes from the model fitted without that run's lot. Both calls answer only for the public wafers, because only they are measured; a synthetic run's depth call is a 404, and so is any call made before three lots have measured wafers. Each fits its models on the request, ten lots left out in turn with the penalty chosen inside each, which answers in 150 to 200 ms on the public data. On the 89-point file the depth loss grows from 0.27 µm at wafer 4 to 0.90 µm at wafer 10, and the mean drift score rises from 0.8 over wafers 1 to 3 to 1.36 at wafer 8.

### Web app

`frontend/` is a React app that reads only the HTTP API. Every response goes through a zod schema where it enters the app, so a field the server stops sending fails in one place instead of deep inside a page. The charts are SVG, with no chart library. The run page draws each trace bucket's lowest and highest reading against the good-run band, with the excursions shaded. The lot page plots a channel's phase mean by wafer against the band, with the fitted line dashed where it projects. The wafer page places every measured site on the wafer, shaded by depth on one blue ramp. The lots page puts the drift score and the depth loss by wafer position in two charts, never on two axes of one. The pointer or the arrow keys read one value at a time, and every chart has a table view. The runs and lots pages switch between the public wafers and the simulated ones, and a simulated run's page opens with the fault the simulator put in, what it did to the channel, and whether the detectors named that channel first and how long after the fault began. `npm run e2e` builds the app and drives the built bundle in Chromium, answering API calls with responses captured from the real API, so the tests need no backend.
...
The other tables are `lot`, `channel`, `ingest_file` (the ingest ledger), `run` (natural key, lot, position, the alignment report, label and label sequence), `run_phase_summary` (count, mean, spread, minimum and maximum of each channel per phase over the scored cycles, summed about the phase's first reading so a phase that never changes has a spread of exactly 0), `measurement` (keyed by run, set and point number, with depth as a generated column `stepheight_um - postox_um`), `injected_fault` (the fault the simulator put into a synthetic run: kind, channel, start, end and size, so a run page can show what went in next to what was caught), `baseline` (unique fingerprint and the settings it was fitted with), `current_baseline` (one row per source), `baseline_good_run`, `baseline_channel` (each channel's role and its longest good-run hold), `summary_band`, `run_assessment` (flag counts, the largest persistent z, and the first channel with the excursion or hold that put it first), `channel_verdict`, `excursion` and `hold`. Assessments, verdicts, excursions and holds are keyed by baseline, so a refit writes new rows beside the old ones, and moving the `current_baseline` row makes them current. That move is a single upsert that applies only when the stored `labels_seq` is not newer, so two refreshes can race without a lock.
...
| Package | Owns | Framework types | Paradigm |
|---|---|---|---|
| `recipe` | Grid, channel names, raw and aligned runs, the aligner, phase summaries | none | Functional core: the aligner is a pure function from a raw run to an aligned one, over records |
...
The stuck rule catches what a band cannot: a sensor that stops updating and repeats one value inside the band. When a baseline is fitted, every channel keeps the longest run of one identical value any good run showed inside the etch, over consecutive recorded samples and across phase boundaries, with a recording gap ending a hold. A run is flagged when a channel holds one value for more than twice that, and for at least 10 samples. The factor came from the same held-out experiment as `k`: at 2 none of the 30 held-out good wafers alarm, at 1.5 one does, a heater at its setpoint. A hold is stored like an excursion, with the sample at which it passed the rule as its confirmation, and ranking treats it as a departure, so a stuck sensor ranks by when it stopped updating. [DATA.md](DATA.md#the-stuck-rule) has the calibration and what the rule can see per channel.
...
Ranking puts channels with a confirmed excursion or a hold first, ordered by the slot where they first left the band or stopped updating, then larger peak `|z|`, then name. The rest follow by largest summary `|z|`. The earliest departure ranks first because later departures are often its effects: a stuck SF6 flow first, the foreline pressure after.
...
`Evaluation.run(EvaluationConfig, SimulationTemplate)` is pure and needs no database. It fits a baseline on 30 clean simulated runs, the first 3 wafers of 10 lots, then generates 1,000 test runs in lots of 10 with 125 faulted, 25 per kind, starting between cycle 5 and cycle 90. Each run is simulated, aligned, scored and dropped, so memory stays small.
...
The five fault kinds are a gas flow stuck low, a pressure spike, a reflected power rise, a sensor dropout to zero, and a sensor that repeats its last reading. A flow stuck below half its setpoint also hides its phase marker, so the aligner predicts onsets and marks the run degraded. That is evidence too, and the run page shows it. A stuck flow is the one fault with a knock-on: the foreline pressure falls with the missing gas load, by the factor and over the three samples measured on the public wafers, while the chamber pressure, which the tool holds at a setpoint, stays put.

`simulate-lot` stores one such lot for the web app, with the truth beside it. It stores the clean wafers 1 to 3 of lots 1 to 10, the training runs the evaluation fits on, then a demo lot whose first 3 wafers are clean and whose other 7 carry five faults, one of each kind, on wafers drawn from the seed and the lot number in the evaluation's size ranges. Each faulted run gets an `injected_fault` row, written in the same transaction as its samples, so the samples and the truth about them cannot disagree; a run stored earlier keeps its samples and its truth whatever the simulator would draw for it today. The refresh at the end fits the synthetic baseline on 33 good runs, the training wafers plus the demo lot's first 3, as the good-run policy would for any lot. A run key carries its seed, so a lot that holds runs from another seed is refused rather than mixed, and a rerun with the same seed finds every run stored, writes nothing and reuses the baseline. With seed 7 and lot 901 all five faults are caught and ranked first on their channel: a 28 W reflected power rise 10.5 s after it starts, an 8 % pressure spike 0.1 s in, a Gas5Flow stuck at 36 % 1.4 s in, with the run degraded because its phase markers went, a HeliumBPPressure dropout at once, and a HeliumBPPressure sensor that repeats its last reading for 6.6 s, seen as a hold of 34 samples that passed the stuck rule 2.7 s in. 3 of the 30 training wafers are flagged, the early-wafer rate the calibration allows. The command takes about 20 s on this PC, and a rerun 5 s.

`FaultSignature` maps the first matching excursion or hold to the kind it looks like, so precision can be counted per kind. The metrics are precision and recall per kind, median detection latency in seconds after the fault starts, the share of clean runs flagged, and how often the rank 1 channel is the injected one. `results/metrics.json` has sorted keys and 4 decimals, and CI fails when a rate moves more than 0.02 or a latency more than 0.5 s. [EVALUATION.md](EVALUATION.md) has the model, the calibration and the results.
...
- Answered: would the first 2 wafers of each lot, closer to the clean, make a tighter reference than the first 3? Measured, no. Wafers 1 to 2 alarm no less when held out, the same two wafers at `k = 6`, and with 20 good runs instead of 30 the summary bands tighten until the tuning capacitor's ordinary rise along a lot is a run-level deviation everywhere: 49 of 96 wafers flagged against 15. Three stays; [DATA.md](DATA.md#choosing-the-thresholds) has the columns.
- The public data has no labeled faults, so the thresholds rest on held-out good wafers from only 10 lots, and the rates carry the sampling error to match: the 2 alarms in 30 at `k = 6` are 6.7 %, anywhere from 0.8 to 22 % at 95 % confidence, and 2.5 to 56 % counted by lot ([DATA.md](DATA.md#choosing-the-thresholds)). More lots would narrow that and could move the thresholds.
- On the public data the limit flags mark a change in the platen match network in four lots, yet those wafers etch no shallower than wafers at the same position in other lots. A flag says the tool changed, not that the wafer is bad, and the run page puts the measured depth beside the flag so a reader sees both.
- Answered: a dropout that holds the last value is invisible to a band detector. The stuck rule above sees it, calibrated on the held-out good wafers to zero alarms, and the simulator injects it as a fifth fault kind, caught 24 times in 25 with precision 1.00. On the public data it adds evidence on two lot 6 wafers whose load capacitor stopped moving, and flags nothing new.
- The gas identities are an inference. The dataset's Readme names the gases, SF6 for etching and C4F8 for passivation, but neither it nor the files say which numbered gas line carries which. The flows and the duty cycle do, and nothing in the code depends on the chemistry.
- The simulator's drift along a lot is the public lots' measured profile by wafer position, so simulated late wafers raise run-level alarms as rarely as the public ones (2.1 % against 0 of 66). It does not model the change of shape within a phase that gives the public late wafers their limit flags.
- The simulator models one knock-on, a stuck gas flow lowering the foreline pressure by the measured 0.21 per sccm, and the flow still ranks above the foreline in every simulated case ([EVALUATION.md](EVALUATION.md#the-committed-evaluation)). Every other simulated channel is independent, so ranking is not tested against the other faults' effects on other channels.
- Answered: a relabel that changes the good runs held the request for about 12 s on the public data. It answers 202 at once now, and the run page polls the refresh until the baseline follows.
