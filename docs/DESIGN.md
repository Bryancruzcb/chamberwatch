...
1. Markers. A sample is SF6-on when Gas5Flow is at least 300, and C4F8-on when Gas4Flow is at least 150 and Gas5Flow is below 300. A step between samples longer than 0.4 s is a gap. `RecipeGrid.gapStepSeconds()` holds that number for the aligner and the limit detector alike.
2. C4F8 phases. A run of C4F8-on samples lasting 0.6 to 2.2 s is a C4F8 phase. The 11.8 s pre-etch step with Gas4Flow at 300 is too long to count. Phase k is the k-th one.
3. SF6 onsets. For cycles 2 to 100, the SF6 onset is the first SF6-on sample after C4F8 phase k-1 ends, so a short dip inside an SF6 phase cannot move the onset. For cycle 1 it is the start of the SF6 run that ends within 0.6 s before C4F8 phase 1, and there may be none.
4. Missing markers. A stuck flow or a dropout can hide a marker. The aligner then walks the cycles like a metronome: the expected onset is the previous onset plus the median of the last five cycle lengths, a detected onset within 0.6 s of it is accepted, and otherwise the expected time is used and counted as predicted.
5. Slots. A sample between an onset and the next onset gets `offset = floor((t - onset) / 0.2 + 0.5)`. Samples between phases stay with the phase that just ended. An offset at or past the phase's capacity marks the sample as overflow and leaves its slot null. Cycle 100's SF6 phase ends when Gas5Flow drops, plus up to three samples, and everything after it is post-etch.
6. Status. `Failed` means a marker channel is missing or no cycle could be anchored. `DEGRADED` means an onset in cycles 2 to 99 was predicted, a gap falls inside the etch, or a sample in cycles 2 to 99 overflowed. Anything else is `ALIGNED`. Degraded runs are scored but never picked as good runs automatically, and the run page shows why a run is degraded.

What the public data gives this aligner, measured before any code was written: 99 C4F8 phases in every wafer; a cycle 1 SF6 phase of 2.8 s in 62 wafers, 4.2 to 4.4 s in 10, and none in 24; SF6 phases of at most 25 samples in cycles 2 to 99 and 30 in cycle 100; C4F8 phases of at most 8 samples; cycles of 5.8 to 6.2 s; recording gaps only after the etch; and one irregular cycle, cycle 74 of lot 3 wafer 7.
...
- Its onset rules. Both sketches assumed 100 C4F8 phases and SF6 phases of 3.5 to 5.5 s, which is what the first written notes on the data said. The files show 99 C4F8 phases, a cycle 1 SF6 phase of 2.8 s or none in most wafers, and a last SF6 phase of 5.4 to 5.6 s. As written, both aligners would have degraded or rejected every public wafer. The alignment section above is rebuilt from the measured structure.
...
Build `RecipeGrid`, `RawRun`, `AlignedRun` and `Aligner` test-first against hand-built runs: a 2.8 s cycle 1 SF6 phase, no cycle 1 SF6 phase, an SF6 phase split by a dip, a stuck SF6 flow, and a gap after the etch. Then add the netCDF reader and run `align` over the 96 public wafers, requiring 96 aligned runs, 99 C4F8 phases each, no overflow in cycles 2 to 99, and exactly one irregular cycle.
