...
Ranking puts channels with a confirmed excursion or a hold first, ordered by the slot where they first left the band or stopped updating, then larger peak `|z|`, then name. The rest follow by largest summary `|z|`. The earliest departure ranks first because later departures are often its effects: a stuck SF6 flow first, chamber pressure after.
