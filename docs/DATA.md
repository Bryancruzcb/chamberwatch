...
The dataset does not label its gas lines. Its Readme names the gases, SF6 for etching and C4F8 for passivation, in 100 cycles of 4.5 s and 1.5 s, and names the tool, an SPTS Omega i2L DSi Rapier. The telemetry has only numbered lines, Gas1Flow to Gas8Flow, and neither the Readme nor the files say which line carries which gas. Gas5 as SF6 and Gas4 as C4F8 is therefore an inference: Gas5Flow holds 600 sccm for the long phase of each cycle and Gas4Flow holds 300 sccm for the short one, which matches the recipe's duty cycle and nothing else in the record does. Nothing in the code depends on the chemistry, only on which line marks which phase, so a wrong guess would swap two labels and change no result.
...
These rates rest on 30 wafers from 10 lots, so they carry a wide sampling error, and the table should be read with it. At the defaults, 2 limit alarms in 30 held-out good wafers is 6.7 %, and an exact 95 % interval for the rate behind it runs from 0.8 to 22 %. The two alarms are in two lots, and wafers of one lot are not independent draws: counted by lot it is 2 of 10, with an interval from 2.5 to 56 %. No run-level alarm in 30 wafers puts that rate under 9.5 % with 95 % confidence, and under 26 % counted by lot. At `k = 5` the 4 of 30 is 13 %, between 3.8 and 31 %, so the data cannot tell 5 from 6 apart with any confidence; 6 is the default because it is the smallest threshold under 10 percent, not because 6.7 % is a measured false alarm rate. A tool with more lots on record would set its thresholds from a tighter version of the same table.
