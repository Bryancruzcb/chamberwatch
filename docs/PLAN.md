...
The dataset's ten daily files hold the plasma's emission spectra, 3,648 wavelengths from 185.89 to 883.97 nm at about 23 Hz with irregular timestamps, 476 to 834 MB per file and 7.9 GB in all, dictionary-encoded like the telemetry. This is the data work the design set aside.

- ~~A Spring Batch job~~ **Not Spring Batch.** This plan asked for one, and building it showed why [DESIGN.md](DESIGN.md#ingest) had already refused it: Batch's restart lives in a job repository of six tables, beside an ingest ledger that already restarts this work from the data itself, and two sources of truth can disagree. The chunk the plan imagined does not fit either, since one wafer group is about 122 MB of codes against a 640 MB container. The job reads one daily file at a time and one wafer at a time, in blocks of samples, verifies each file against the committed checksums first, and records a file in the ingest ledger and a reduction's version on each run, so a crash resumes at the next wafer and a rerun reads nothing.
- The reduction: five emission lines chosen after measuring the spectra, three fluorine and two C2 Swan band heads, each summed over three pixels and averaged into the recipe's 0.2 s slots. The candidates the plan guessed at did not all survive the measurement: the file's wavelength axis reads 1.23 nm low, nothing below 428 nm carries signal, and a single pixel samples where a line falls on the grid rather than measuring the line. The records are aligned by the plasma, which both of them show.
...
| 5 emission spectra | #31, #32 | done: five lines reduced into the recipe's slots as channels of the run; 84 of 96 wafers stored, 12 left out because the spectrometer skipped through their etch; 7.4 GB read in 3 min 15 s, a rerun writes nothing in 18 s. Refitting on them changes no flag at all, and they add 0.006 µm to a depth prediction, which is noise |
| 6 chamber simulator | #33, #34, #35 | done: `chamber-sim/` streams a wafer (21,320 checks, clean under the sanitizers, the same run on Windows and Linux) and ChamberWatch records it: a run the C program etched is stored, aligned, banded and scored under a `LIVE` baseline with its injected fault beside it, checked against the real binary in `SimulatorBinaryTest`; the Live page watches a run arrive, and a lot streamed as three clean wafers then a faulted one gets the fault flagged with its channel named first |
