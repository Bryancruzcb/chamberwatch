...
|---|---|---|---|
| Gas flow stuck low | Gas4Flow, Gas5Flow | 30 to 95 % of the flow | from its start to the end of the etch |
...
- **Channel coupling.** Channels are independent, so ranking is tested against unrelated departures, never against a fault's own knock-on effects.
